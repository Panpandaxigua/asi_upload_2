...
macOS 版本只改变硬件码的来源：只读取主板级 `IOPlatformUUID`（出厂固定、每台唯一）。客户修改电脑名称、升级 macOS 或以 Rosetta 方式运行都不会导致硬件码变化，无需重新激活。激活码格式、签名校验、产品 ID、许可证文件加密逻辑都保持不变。

> 注意：2026-09-08 之前的旧 Mac 版本硬件码包含主机名和系统版本，升级到本版本后旧激活会失效，需要客户重新提供硬件码并重新生成激活码（一次性）。
